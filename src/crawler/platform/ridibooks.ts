...
export async function get60WebtoonRanking(type: TYPE, page: number) {
  try {
    const { data } = await axios({
      method: 'get',
      url: `https://ridibooks.com/_next/data/3.8.181-2b88d02/category/books/${type}.json?tab=books&category=${type}&page=${page}&order=review`,
    });
...
export async function get20BestRanking(gerne: GENRE) {
  ...
  try {
    const { data } = await axios({
      method: 'get',
      url: `https://ridibooks.com/_next/data/3.8.181-2b88d02/bestsellers/${gerne}.json?genre=${gerne}`,
    });
